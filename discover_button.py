...
p_robot_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
p_u_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
p_r_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
p_d_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
p_l_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
game_dict = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
game_robot_dict = {}

#update p_robot_dict
for pos,pod in robot_dict.items(): #for each position robot can take on
    for obj,objd in pod.items(): #for each potential successor object
        ...
        for dyn,counts in objd.items(): #now populate p_robot_dict with probabilities
            p_robot_dict[pos][obj][dyn] = float(counts)/total_counts
        

#update p_u_dict
for pos,pod in u_dict.items(): #for each position obj can take on
    for obj,objd in pod.items(): #for each potential successor object
        total_counts = 0
        for dyn,counts in objd.items(): #for each dynamic successor objecy can take on
            total_counts += counts
        for dyn,counts in objd.items(): #now populate p_robot_dict with probabilities
            p_u_dict[pos][obj][dyn] = float(counts)/total_counts
print(robot_dict[(1,1)]['u'])
...
#demonstrate how to convert this into transition function (Hint, it's really easy, just normalize the above probabilities and sample from them when applying update dynamics according to object_dynamics, and make sure that in order to avoid active dynamic conflicts to resolve the object update only when all dynamic likelihoods have been calculated, and choose most likely)
#use transition function in VI
#plug entire thing together and look at results!
